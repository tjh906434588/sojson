// 创建路由：/ 挂载默认布局（DefaultLayout），各一级菜单路由模块作为其子路由
// 项目无独立首页，根路径 / 直接重定向到第一个一级菜单（JSON 工具）
// 各菜单的路由、路径常量、落地页卡片、NavBar 菜单均在对应模块（modules/）中定义，业务侧从对应模块引入
import { createRouter, createWebHistory, type RouteRecordRaw } from 'vue-router'
import Layout from '../layout/index.vue'
import { ROUTE_JSON } from './modules/json'
import { jsonRoutes } from './modules/json'
import { encryptRoutes } from './modules/encrypt'
import { compressRoutes } from './modules/compress'
import { documentRoutes } from './modules/document'
import { frontendRoutes } from './modules/frontend'
import { convertRoutes } from './modules/convert'
import { otherRoutes } from './modules/other'

const routes: RouteRecordRaw[] = [
  {
    path: '/',
    component: Layout,
    children: [
      // 根路径默认导航到第一个一级菜单（JSON 工具）
      { path: '', redirect: ROUTE_JSON },
      ...jsonRoutes,
      ...encryptRoutes,
      ...compressRoutes,
      ...documentRoutes,
      ...frontendRoutes,
      ...convertRoutes,
      ...otherRoutes
    ]
  }
]

const router = createRouter({
  history: createWebHistory(),
  routes
})

export default router
